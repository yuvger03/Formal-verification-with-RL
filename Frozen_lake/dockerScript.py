...
for i in range(10):
    for j in range(3):
        os.system(f"docker create --name c{i*10+j*3} --rm -v $HOME:$HOME -v /opt/gurobi:/opt/gurobi -w $HOME/softmax ubuntu_python3.8:1 python3 main.py {0.99 - 0.01*j} {1-i*0.01} {i*10+j*3}")  # This is the command that opens the docker container
